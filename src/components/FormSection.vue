    ...
    <main class="max-w-6xl pl-[200px] mt-10 px-4">
      <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div class="bg-white rounded-2xl p-6 border border-black-100">
            <form @submit.prevent="submitForm" class="space-y-4">
              ...
              <div class="flex flex-col">
                ...
                <select v-model="form.gender" class="w-full border rounded-lg p-2 text-sm text-gray-900" required>
              ...
              <div class="flex flex-col">
                ...
                <input type="text" v-model.number="form.age" inputmode="numeric" pattern="[0-9]*"
                  class="w-full border rounded-lg p-2 text-sm text-gray-900" placeholder="Masukkan umur anda (Tahun)"
                  @keydown="allowOnlyNumbers" required />
              </div>
  
              <div class="flex flex-col">
                <label class="block text-sm text-gray-600 mb-1">Pekerjaan</label>
                <select v-model="form.job" class="w-full border rounded-lg p-2 text-sm text-gray-900" required>
                  <option value="" disabled hidden>Pilih jenis pekerjaan Anda</option>
              ...
              <div class="flex flex-col">
                ...
                <select v-model="form.salary" class="w-full border rounded-lg p-2 text-sm text-gray-900" required>
                  <option value="" disabled hidden>Rentang penghasilan per bulan</option>
              ...
              <div class="flex flex-col">
                ...
                <select v-model="form.location" class="w-full border rounded-lg p-2 text-sm text-gray-900" required>
                  <option value="" disabled hidden>Pilih provinsi domisili</option>
                  <option v-for="province in provinces" :key="province.id" :value="province.text">
                    {{ province.text }}
                  </option>
                </select>
              </div>
              ...
              <div class="flex items-start space-x-2">
                ...
                <label class="text-xs text-gray-600">
                  ...
                  <a @click.prevent="showModal = true" href="#" class="text-teal-500 underline cursor-pointer">
                    Kebijakan Privasi
                  </a>
                </label>
              </div>
                ...
                <div class="bg-white rounded-2xl p-6 w-[700px] h-[498px] shadow-lg relative overflow-y-auto">
                  ...
                  <div class="text-sm text-gray-600 leading-relaxed space-y-4">
                    ...
                    <ul class="list-disc list-inside ml-4">
                      <li>Umur</li><li>Jenis Kelamin</li><li>Range Gaji</li><li>Jenis Pekerjaan</li><li>Domisili</li>
                    </ul>
                    <p><strong>2. Tujuan Pengumpulan Data</strong></p>
                    <ul class="list-disc list-inside ml-4">
                      <li>Analisis statistik dan pengembangan layanan.</li><li>Personalisasi rekomendasi untuk pengguna.</li>
                    </ul>
                    ...
                    <ul class="list-disc list-inside ml-4">
                      <li>Memeriksa data mereka yang tersimpan.</li><li>Meminta perubahan atau penghapusan data.</li><li>Mengetahui bagaimana data mereka digunakan.</li>
                    </ul>
        ...
        <div v-if="recommendations.length">
          <h4 class="text-xl font-semibold text-gray-800 mb-4 border-b pb-2 text-center">
            Kami Menemukan Produk Terbaik untuk Kamu
          </h4>
          <div class="grid grid-cols-3 gap-4">
            <div v-for="(product, index) in recommendations" :key="index" class="rounded-lg border border-gray-200 overflow-hidden shadow-sm">
              <img src="../assets/emas-rekomendasi.png" alt="Rekomendasi" class="w-full h-32 object-cover" />
              <div class="p-2">
                <h5 class="font-medium text-sm text-gray-800">{{ product }}</h5>
                <p class="text-xs text-gray-600">Solusi keuangan sesuai pilihanmu</p>
              </div>
            </div>
          </div>
        </div>
  ...
  const submitForm = async () => {
    try {
      const body = JSON.stringify({
        job: form.job, age: form.age, province: form.location.toLowerCase(),
        gender: form.gender, salary: form.salary
      });
